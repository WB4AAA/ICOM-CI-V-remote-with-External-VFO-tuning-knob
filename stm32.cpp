...
DWORD dwSize = 0;
       ...
       for(int i = 0; SetupDiEnumDeviceInfo(hDevInfo, i, &spdd); i++)
       {
        ...
        if ((r1==0)&&(r2==0))
        {
          ...
          SetupDiEnumDeviceInterfaces( hDevInfo, NULL, &ClassGuid, dwIndex, &spdid ) ;
          ...
          SetupDiGetDeviceInterfaceDetail(hDevInfo, &spdid, pspdidd, dwSize, &dwSize, NULL );
...
//------------------Scan Port VID and PID to check if Port still available------
bool PortScan()
{
GUID ClassGuid={0x86E0D1E0, 0x8089, 0x11D0, 0x9C, 0xE4, 0x08, 0x00, 0x3E, 0x30, 0x1F, 0x73};
HDEVINFO hDevInfo = SetupDiGetClassDevs(&ClassGuid, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE );
...
//--------------------Thread to Listening Serial Port for Data------------------
DWORD WINAPI ReadPort(LPVOID)
{
        ...
        while (Connected==true)
        {
            ...
            if (btr > 0)
            {
              ...
              //if (ok && BytesRead > 0)
              if ((ok==true)&&(KnobRead>0))
              {
                //Decode(buffer);
                memcpy(knobData, buffer, 4);
              }
              //portFree=true;
            }
            Sleep(1);
...
//--------------------Create COM port by STM32 VID and PID----------------------
bool PortCreate()
{
...
//-------------------Scan for VID and PID of connected port---------------------
...
//-------------------------Main Function to Start USB Knob----------------------
void KnobInit()
{
   Connected=false;
